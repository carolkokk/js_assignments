...
for (let i=0;i<picArray.length;i++){
  ...
  article.addEventListener('click',function(evt){
    const clicked = picArray[i];
    const dialog = document.querySelector('dialog');
    document.querySelector('dialog img').src = clicked.image.large;
    document.querySelector('dialog img').alt = clicked.title;
    dialog.showModal();
  });
}
...
document.querySelector('dialog span').onclick = closeDialog;